...
import { requestThumbnail, isCancellation } from "@services/images/thumbnail-service";
import {
  getHdImage,
  loadHdImage,
...
export class PfImageCanvas extends LitElement {
  ...
  * Updated by the canvas state adapter; redraws on change. */
  @state()
  private postProcess: PostProcessSettings = canvasState.getPostProcess();
  ...
  private cropEnabled(): boolean {
    return canvasState.cropEnabled(this.path);
  }
  ...
  * mode would produce. */
  ...
  async exportJpeg(): Promise<Uint8Array> {
    ...
    const photoValues: Record<string, unknown> = this.previewOriginal
      ? {}
      : { ...canvasState.readValues("photo", path) };
