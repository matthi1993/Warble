...
export function readCurveToolValue(scope: ToolScope, path: string | null, state: EditorState): CurveEdit {
  if (scope === "post") return state.getPostProcess().curve;
  return (path ? state.getPhotoEdit(path)?.curve : null) ?? defaultCurve();
}
...
export class CurveTool extends EditTool {
  ...
  private value(host: ToolHost): CurveEdit {
    return readCurveToolValue(this.scope, host.editTarget, this.state);
  }

  hasEdits(target: string): boolean {
    return !isCurveZero(this.state.getPhotoEdit(target)?.curve);
  }

  serializeEdit(target: string): unknown | null {
    const value = this.state.getPhotoEdit(target)?.curve;
  ...
  applyEdit(host: ToolHost, data: unknown): void {
    ...
    this.state.setPhotoCurve(host.editTarget, value && !isCurveZero(value) ? value : null);
    host.requestUpdate();
  }

  reset(host: ToolHost): void {
    if (this.scope === "post") this.state.resetPostCurve();
    else if (host.editTarget) this.state.setPhotoCurve(host.editTarget, null);
    host.requestUpdate();
  }
  ...
  renderCard(host: ToolHost): TemplateResult {
    return html`<pf-curve-card
      ...
      @curve-change=${(event: CustomEvent<CurveEdit>) => {
        if (this.scope === "post") this.state.setPostCurve(event.detail);
        else if (host.editTarget) this.state.setPhotoCurve(host.editTarget, event.detail);
        host.requestUpdate();
      }}
